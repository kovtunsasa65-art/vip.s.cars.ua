...
function ScrollAnimatedButtons() {
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      <motion.a href="https://t.me/vips_cars" target="_blank" rel="noreferrer" style={{ x, y, scale }} className="absolute w-14 h-14 bg-blue-500 text-white rounded-full flex items-center justify-center shadow-lg shadow-blue-500/20 pointer-events-auto cursor-pointer" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
        <MessageCircle size={24} />
      </motion.a>
      <motion.a href={PHONE_TEL} className="absolute w-16 h-16 bg-green-500 text-white rounded-full flex items-center justify-center shadow-xl shadow-green-500/20 pointer-events-auto cursor-pointer" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} initial={{ scale: 0 }} animate={{ scale: 1 }}>
...
function HomePage() {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <SEOHead title="Автопідбір та викуп авто в Києві" description="Підберемо авто без ризику або викупимо за 1 день. Перевірені авто з Індексом Довіри 0–100. Київ." url="/" schema={orgSchema()} />
      <Hero />
      <FeaturedGallery />
      <Services />
      <Catalog />
