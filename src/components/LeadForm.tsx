...
import { Phone, User, DollarSign, Car as CarIcon, Send, Sparkles, CheckCircle2 } from 'lucide-react';
...
export default function LeadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Зберігаємо в таблицю leads
      const { error } = await supabase
        .from('leads')
        .insert([{
          type:    formType === 'selection' ? 'підбір' : 'викуп',
          name:    formData.name,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-12 xl:col-span-7">
            <div className="bg-slate-50 rounded-3xl p-8 md:p-12 border border-slate-200">
              {/* Type Toggle */}
              <div className="inline-flex bg-white rounded-xl p-1.5 border border-slate-200 mb-10 w-full md:w-auto overflow-hidden">
                <button
                  onClick={() => setFormType('selection')}
                  className={cn(
                    "px-10 py-3 rounded-lg text-sm font-bold transition-all flex-1 md:flex-none uppercase tracking-wider",
                ...
                <form className="space-y-8" onSubmit={handleSubmit}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="space-y-2">
                        ...
                        <input
                          ...
                          className="w-full bg-white border border-slate-200 rounded-xl px-5 py-4 font-semibold text-slate-900 focus:outline-none focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/5 transition-all outline-none"
                        />
                     </div>
                     <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                     ...
                     <textarea
                       ...
                       placeholder={formType === 'selection' ? 'Яку модель шукаєте? Який ваш бюджет?' : 'Яке авто продаєте? Рік, пробіг, стан?'}
                       className="w-full bg-white border border-slate-200 rounded-xl px-5 py-4 font-semibold text-slate-900 min-h-[120px] focus:outline-none focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/5 transition-all outline-none resize-none"
                     />
                  </div>

                  <button 
                    disabled={isSubmitting}
                    className="w-full bg-brand-blue text-white py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-brand-blue-dark transition-all shadow-xl shadow-brand-blue/25 disabled:opacity-50 disabled:cursor-not-allowed group flex items-center justify-center gap-3"
                  >
